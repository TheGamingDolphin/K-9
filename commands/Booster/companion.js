...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("companion")
    .setDescription("[BOOSTER ONLY] Become a random companion"),
  async execute(interaction) {
    ...
    if (role && role.id === "1018200127598497893") {
      ...
      if (member.roles.cache.has(role.id)) {
          ...
          if (canNickname === true) {
            ...
            if (nicknameWithUsername.length <= 32) {
              ...
              interaction.member.setNickname("Rose Tyler").catch((error) => {
                console.error(
                  "Error setting nickname without username:",
                  error
                );
              });
            }
        ...
        await interaction.editReply(
          "You need the `Dalek` (booster) role to be able to use this command!"
        );
      }
    }
  },
